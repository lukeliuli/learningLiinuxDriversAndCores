...
#define DRIVER_DESC "csust-cekong usbserial(cp210x,0x10C4, 0xEA88) driver ; drivierSilicon Labs CP210x RS232 serial adaptor driver"
		...
		struct usb_serial_port *port, struct ktermios *old_termios)
{
	...
	if(baud != 9600)
	  {
		  printk("baud is fixed and can not change");
		  baud = 9600;
	  }
	else
	  {
		  printk("baud is fixed and the setting is OK");
	  }
